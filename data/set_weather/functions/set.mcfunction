...
execute if score weather.control spc.weather.set matches 2 run tellraw @a [{"text":"天气状态： ","hoverEvent":{"action":"show_text","value":{"text":"/weather thunder"}}},{"text":"[雷雨]","color":"aqua","clickEvent":{"action":"run_command","value":"/function set_weather:infoshow"},"hoverEvent":{"action":"show_text","value":{"text":"永久雷雨"}}}]
tellraw @s [" "]
